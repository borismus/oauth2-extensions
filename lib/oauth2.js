...
OAuth2.prototype.finishAuth = function() {
  ...
  that.getAccessAndRefreshTokens(authorizationCode, function(at, rt, exp) {
    ...
    if (rt) that.set('refreshToken', rt);
...
/**
 * Attempt to refresh the access token if it has expired and a refresh token
 * exists.
 *
 * This method is almosts the same as authorize except that it won't create
 * access tokens unless one already exists and only refreshes expired access
 * tokens when a refresh token is available.
 *
 * This allows you to lazily ensure existing access tokens are up-to-date but
 * does not open an authorization popup which is useful when called just before
 * calling the endpoint.
 *
 * @param {Function} [callback] The method to be called when refresh is
 * complete. This method is passed a single argument indicating whether or not
 * the access token is refreshed.
 */
OAuth2.prototype.refreshIfRequired = function(callback) {
  var that = this;
  OAuth2.loadAdapter(that.adapterName, function() {
    that.adapter = OAuth2.adapters[that.adapterName];
    var data = that.get();
    if (data.accessToken && that.isAccessTokenExpired()) {
      // There's an existing access token that has expired.
      if (data.refreshToken) {
        that.refreshAccessToken(data.refreshToken, function(at, exp) {
          that.set('accessToken', at);
          that.set('expiresIn', exp);
          that.set('accessTokenDate', (new Date()).valueOf());
          // Invoke the callback method when we finish refreshing and tell
          // caller that all is well.
          if (callback) callback(true);
        });
      } else {
        // No refresh token so ensure the caller knows manual refresh is still
        // required.
        if (callback) callback(false);
      }
    } else {
      // Refresh isn't required so notify caller that they can continue.
      if (callback) callback(true);
    }
  });
};
