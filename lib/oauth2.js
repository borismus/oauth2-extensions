...
OAuth2.prototype.openAuthorizationCodePopup = function(callback) {
  ...
  // Remember the current tab id so we later can switch back to the right one
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    window['last-tab-id'] = tabs[0].id;
  });
...
OAuth2.prototype.finishAuth = function() {
  ...
  function callback(error) {
    ...
    chrome.tabs.update(background_page['last-tab-id'], {selected: true});
